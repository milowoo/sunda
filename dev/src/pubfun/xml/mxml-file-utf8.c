/*
 * "$Id: mxml-file-utf8.c,v 1.1 2009/07/07 12:37:30 nscard Exp $"
 *
...
static const char * _write_ws_cb(mxml_node_t * node, int where);
	   ...
	   mxml_save_cb_t cb)		/* I - Whitespace callback or MXML_NO_CALLBACK */
{
  ...
  global->wrap = 0x7F000000;
  if( cb == MXML_NO_CALLBACK )
  	cb = _write_ws_cb;
...
const char * _write_ws_cb(mxml_node_t * tree, int where)
{
	mxml_node_t * node;
	static char fill[100];
	int level;
	...
	case MXML_WS_BEFORE_CLOSE:
		for( node=tree->child; node!=NULL && node->type != MXML_ELEMENT; node=node->next );
		if( node )
		{
			for( level=1,node=tree->parent; node!=NULL && node->parent!=NULL && level<30; node=node->parent,level++ )
				strcat( fill,"  " );
		}
		break;
	case MXML_WS_BEFORE_OPEN:
		for( node=tree->prev; node!=NULL && node->type != MXML_ELEMENT; node=node->prev );
//		if( node == NULL && tree->parent != NULL && tree->parent->parent != NULL)
		if( node == NULL && tree->parent != NULL )
			strcpy( fill,"\n" );
		for( level=1,node=tree->parent; node!=NULL && node->parent!=NULL && level<30; node=node->parent,level++ )
			strcat( fill,"  " );
...
//		if (!strncmp(tree->value.element.name, "?xml", 4))
//			strcpy( fill,"\n" );
		break;
	default:
		break;
...
/*
 * End of "$Id: mxml-file-utf8.c,v 1.1 2009/07/07 12:37:30 nscard Exp $".
 */
